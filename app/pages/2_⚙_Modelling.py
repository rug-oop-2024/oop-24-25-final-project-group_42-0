...
if current_dataset_name:
    current_dataset = Dataset.from_artifact(datasets[names.index(current_dataset_name)])
    ...
    chosen_preffered_feature = st.selectbox("choose preffered feature", chosen_features, None)
...
if chosen_model is not None:
    ...
    if current_model.type == "classification":
        chosen_metrics: list = st.multiselect("choose metrics (optional)", CATEGORICAL_METRICS, None)
    elif current_model.type == "regression":
        chosen_metrics: list = st.multiselect("choose metrics (optional)", CONTINUOUS_METRICS, None)

    current_metrics = [get_metric(metric) for metric in chosen_metrics]

if current_model is not None and st.button("run"):
    pipeline = Pipeline(current_metrics, current_dataset, current_model, list_of_features, chosen_preffered_feature)
    st.write(pipeline)
